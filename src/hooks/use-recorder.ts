...
/** The translator-facing sentence for each `classifyStopDecode` error class. The
 *  classifier stays UI-free; this is the one place the case becomes words. */
...
/** The honest sentence for each refusal (#203). These used to be typed out here
 *  because `hooks/` cannot reach `components/`; the table moved to `lib/i18n`
 *  in #169, so the copy lives with the rest of the app's words again. */
  ...
  const retryDecode = useCallback(
    async (blob: Blob): Promise<RetryDecodeResult> => {
      ...
      try {
        ...
        if (samples.length === 0) {
          return { samples: null, error: strings.captureSilence };
        }
        return { samples, error: null };
      } catch {
        return {
          samples: null,
          error: strings.captureUndecodable,
        };
      }
    },
    []
