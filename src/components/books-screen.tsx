...
export function BooksScreen({
  ...
  useEffect(() => {
    ...
    // Add-chapter prompt sets the pending scroll and closes itself in one async
    // continuation alongside the hook's own `setBooks`, and this hand-off must
    // not depend on React batching those into one commit.
  }, [books, deleteTargetId, newBookSeed, newChapter, scrollPending, nodeFor]);
  ...
  const onConfirmNewChapter = useCallback(
    async (typed: string) => {
      ...
      if (!chapter) {
        newChapterReturnFocus.current =
          nodeFor(bookId)?.querySelector<HTMLElement>("button") ??
          newChapterReturnFocus.current;
        return;
      }
    ...
    [addChapter, layers, newChapter, nodeFor]
  );
