...
/**
 * What App (slice 4) can drive from outside (#374/#393): whether Books' OWN
 * hamburger menu, New Book dialog, book ≡ menu (rename/share), or delete
 * confirm currently owns a system Back gesture, and how to dismiss it if so —
 * the Books half of the same nav-layer fix the recorder already has via
 * `overlayBlocksClose`/`overlayDismissal`. Books has no per-row overlay the
 * way Segments does, so unlike `SegmentsScreenHandle` this covers everything
 * the shelf can show at once.
 */
export interface BooksScreenHandle {
  /** True while any of Books' own overlays is open. */
...
interface BooksScreenProps {
  ...
  /**
   * ANY Books overlay just opened/closed (#374). App pushes/consumes ONE
   * history entry across that transition — Books otherwise pushes none at all,
 ...
 */
export const BooksScreen = forwardRef<BooksScreenHandle, BooksScreenProps>(
  function BooksScreen({ onOpenChapter, onOverlayOpen, onOverlayClose }, ref) {
    const {
      books,
      newBookPlaceholder,
      loading,
    ...
    const shareMenuBook =
      books.find((b) => b.bookId === shareMenuBookId) ?? null;
    ...
    // #374 — every overlay this screen can show at once: the hamburger Menu, New
    // Book, the book ≡ menu (rename/share), and the delete confirm. Only one is
    ...
    useImperativeHandle(
      ref,
      () => ({
        hasOpenOverlay: () => hasBooksOverlay,
        dismissOverlay: () => {
          ...
          // `lib/nav/navigation.ts`). New Book (`onCancelNewBook`) and a rename's
    ...
    return (
      ...
      >
        ...
        {/* Two standing background conditions can be true at once — the browser
          ...
          These two lines render INDEPENDENTLY of the exclusive slot above —
          not because of it (#406 item 1: the earlier wording claimed a gate
          here that does not exist). `storage` is null until
          `useStoragePersistence`'s own `hasContent` argument
          (`loaded && books.length > 0`) is true; `encoderLine` is null
          whenever `encoderHealth()` reads `"ok"`, which is the module's
          default and stays true until an actual encode has failed —
          neither reads `noticeText` or `loading` at all. The two rarely show
          alongside a load failure or a loading spinner in practice only
          because an unloaded/errored shelf usually has no books to satisfy
          `hasContent` with; that is a consequence of `hasContent`, not
          something coded in this slot.
