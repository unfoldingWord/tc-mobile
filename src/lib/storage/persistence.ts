 ...
 * is nothing to say.
 *
 * Three inputs, not two. `undefined` is **unknown**, and unknown is silence:
 * `navigator.storage` may not exist (the iOS Safari case), and a query can
 * reject. Writing this as `!persisted` would put an eviction warning in front of
 * every translator on a device we simply could not ask — a claim with no
 * evidence behind it, on the one platform this repo has actually run on. Only an
 * explicit `false` — the browser saying it has not promised to keep this data —
 * earns the marker.
 */
