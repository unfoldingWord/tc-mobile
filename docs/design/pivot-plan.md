...
| **OBS media cache** (`hooks/obs-media.ts`, `lib/storage/media.ts`, the `media` store in `db.ts`) — **added to B0 24 Aug**            | ~180  | Q4 answered no. Zero live consumer, no mockup shows artwork, no batch scheduled to wire it. Closes #1 as moot. `DB_VERSION` stays 2; the stale store is swept by B1's drop-and-recreate. |
