...
- **#601** (v1-required) — Segments row: after a run-out the dot rests at ≈1.0 and every next Play starts at the end. Code-read confirmed (`segment-row.tsx:325` passes `fraction × duration`). The recorder does *not* have this defect (`panOrRest` → `null` at length → whole buffer).
...
5. **#245 rows R1–R6** are dated 09-24/25 with *requested* owners; confirm them or they lapse.
