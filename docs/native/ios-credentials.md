...
| **Access / role** | **App Manager** (Admin also works) | Required by `README.md` §4a and the Fastfile header. `xcodebuild -allowProvisioningUpdates` **creates the certificate and provisioning profile on the fly** — the archive self-signs (automatic, `iPhone Developer`), and the export phase (`export_options` `method: app-store`) mints the **distribution** pair. A `Developer`-role key cannot, and the failure surfaces late — in the export phase, as a signing error rather than a permissions one. |
