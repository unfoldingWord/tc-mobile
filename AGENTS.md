...
1. **MP3 encoding is off the main thread** since B8 (ADR 0009): one Web Worker,
   ...
   service-worker update has purged (#192, ADR 0009 amended again); the snapshot
   path is browser-only and **has never been constructed in a browser here**, so
   it carries a fallback to the direct chunk URL. What remains from ADR
