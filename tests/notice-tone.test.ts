 ...
 * failure (`alert`), work to wait for (`busy`), and a heads-up (`info`) —
 * covering both something already done and a standing condition such as
 * storage durability (#214/#406) — must never share a mark. Before this the
 * share gap warning ("N segments were left out") rode on `busy`, so it wore
