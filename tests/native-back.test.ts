 ...
 * itself and `exitApp()` need a phone. This file proves the dispatch only.
 */
...
describe("attachNativeBack — the shell's hardware Back (#374)", () => {
  ...
  it("(b) an empty stack on Books: exits the app and issues no history Back", () => {
    const back = fakeBack();
    const goBack = vi.fn();
  ...
  it("(b') an empty stack on Books with a floor entry still standing (#535): exits at once rather than spending a silent Back", () => {
    const back = fakeBack();
    const goBack = vi.fn();
    attachNativeBack(back.plugin, { decide: decideFor("books", []), goBack });
