...
describe("stop() releases the stolen stream and the LOCAL tap in both arms, and its flush-throw path reports, seals what it has and falls through to a tail that returns to idle (#59 #474 R6, #485, panel r1, panel r2)", () => {
  ...
  it('"recorder-stop-flush" is one site in the file, and the tail\'s empty-capture exit picks its sentence on flushThrew (#485, panel r1)', () => {
    ...
    //
    // Both halves are pinned since #169 moved the two sentences into the
    // string table: the source text picks the right KEY, and the table gives
    // that key the right WORDS. Reading only the source would leave the two
    // sentences free to swap places inside `en.ts` with this still green.
    ...
    expect(afterElse).toMatch(
      /blob\.size\s*===\s*0[\s\S]*?flushThrew\s*\?\s*strings\.recordStopFailed\s*:\s*strings\.captureSilence/
    );
    expect(strings.recordStopFailed).toBe("Could not finish this recording.");
    expect(strings.captureSilence).toBe("No sound was recorded. Try again.");
  });
